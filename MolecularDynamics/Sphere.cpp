		...
		: m_lats(lats), m_longs(longs)
	{
		...
		for (unsigned int i = 0; i < lats; ++i) {
			...
			for (unsigned int j = 0; j <= longs; ++j) {
				...
				// vertex 
				buffer[i*(longs + 1) * STEP_SIZE + j * STEP_SIZE + 6 + (useTexture ? 2 : 0)] = static_cast<float>(x);
				buffer[i*(longs + 1) * STEP_SIZE + j * STEP_SIZE + 7 + (useTexture ? 2 : 0)] = static_cast<float>(y);
				buffer[i*(longs + 1) * STEP_SIZE + j * STEP_SIZE + 8 + (useTexture ? 2 : 0)] = static_cast<float>(z0);

				// normal
				buffer[i*(longs + 1) * STEP_SIZE + j * STEP_SIZE + 9 + (useTexture ? 2 : 0)] = static_cast<float>(x / lng);
				buffer[i*(longs + 1) * STEP_SIZE + j * STEP_SIZE + 10 + (useTexture ? 2 : 0)] = static_cast<float>(y / lng);
				buffer[i*(longs + 1) * STEP_SIZE + j * STEP_SIZE + 11 + (useTexture ? 2 : 0)] = static_cast<float>(z0 / lng);
